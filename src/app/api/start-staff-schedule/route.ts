...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const existingStartedAt = await getStaffScheduleStartedAt(supabase, sessionId);
    ...
    const { data: inserted, error: insertErr } = await supabase
      .from("events")
      .insert({ session_id: sessionId, event_type: "staff_schedule_started" })
      .select("created_at")
      .single();
    if (insertErr) throw new Error(`staff_schedule_started insert failed: ${insertErr.message}`);

    return NextResponse.json({ startedAt: inserted.created_at });
  } catch (err) {
    console.error("POST /api/start-staff-schedule failed", err);
